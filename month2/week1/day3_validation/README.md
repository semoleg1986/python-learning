...
default_factory — это способ генерации динамического значения по умолчанию (UUID, datetime.now, random, и т. д.)
💬 «Создай новое значение при каждом создании объекта».

| Параметр        | Поведение                                                                                           |
|-----------------|-----------------------------------------------------------------------------------------------------|
| default         | Значение фиксировано. Например, default=uuid4() — функция вызовется один раз при объявлении класса. |
| default_factory | Функция вызывается каждый раз при создании нового объекта.                                          |

### default

- Определяет значение по умолчанию для поля.
- Если клиент не передал это поле в JSON, Pydantic использует default.
- Влияет на валидацию и работу модели.

### example

- Только для документации Swagger/OpenAPI.
- Показывает пользователю пример значения в Swagger UI или ReDoc.
- Не влияет на значение по умолчанию при создании объекта.
